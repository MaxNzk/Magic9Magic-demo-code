...
public class ObjectTableEditor : EditorWindow
{
    ...
    private const string NAME_ENDS_WITH = "SO";

    [MenuItem("Tools/ObjectTableEditor")]
    static void Open()
    {
        GetWindow<ObjectTableEditor>();
    }
    ...
    private void RetrieveType()
    {
        ...
        var subclassesScriptableObject =
                        ...
                        where (type.IsSubclassOf(typeof(ScriptableObject)) && type.Name.EndsWith(NAME_ENDS_WITH)) // where type.IsSubclassOf(typeof(ScriptableObject))
                        select type;
        ...
        _objectTypeInfos = new TypeInfo[subclassesScriptableObject.Count()];
    ...
    void CreateDisplayFrom(TypeInfo type)
    {
        ...
        // Debug.Log("type.type = " + type.type.ToString());
        if (type.type.ToString() == "InventoryItemSO")
        {
            // Debug.Log("InventoryItemSO");
            // Debug.Log("propCount = " + propCount.ToString());
            propCount -= 2; // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            // Debug.Log("propCount = " + propCount.ToString());
        }
        ...
        for (int i = 1; i < propCount; ++i)
        {
            ...
            // Debug.Log(prop.type.ToString());
            
            prop.NextVisible(false);
            // Debug.Log(prop.displayName.ToString());
...
public class DatabaseDisplayer : TreeView
{
    ...
    protected ObjectTableEditor.TypeInfo _objectType;

    public DatabaseDisplayer(TreeViewState state, MultiColumnHeader header, ObjectTableEditor.TypeInfo objectType) : base(state, header)
    {
        _freeID = 0;
    ...
    protected override void RowGUI(RowGUIArgs args)
    {
        ...
        int argLen = args.GetNumVisibleColumns();
        // argLen -= 2; // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    protected override TreeViewItem BuildRoot()
    {
        ...
        {
            Debug.Log("438: _objectType.isScriptableObject == false");
            objs = new Object[0];
...
public class DatabaseViewerItem : TreeViewItem
{
    ...
    public static DatabaseViewerItem CreateFromUnityObject(UnityEngine.Object unityObject, DatabaseDisplayer treeView)
    {
        ...
        int len = newItem.properties.Length;
        // len -= 2; // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        ...
        for (int k = 1; k < len; ++k)
        {
            ...
            // Debug.Log(newItem.properties[k].name);
            // Debug.Log(prop.name);
            // Debug.Log("---------------------------");
        }
...
public class SerializePropertyComparer : IComparer<SerializedProperty>
{
    ...
    // TODO : look for a better way this probably generate ton of garbage + need to be extended manually
