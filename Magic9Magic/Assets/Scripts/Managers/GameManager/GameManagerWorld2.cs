...
public class GameManagerWorld2 : MonoBehaviour, IPassedThroughPortal, IPortal, IFinishMission, IManaMaker
{
    ...
    private void Awake()
    {
        ...
        SetAnalyticsStepFirstWorld2();
        CreateWorld();
    }

    private void PrintSceneInfo()
    {
        Tools.Log("gameSettingsSO.GameDifficulty = " + _gameStateSettingsSO.GameDifficulty);
    }
    ...
    private void FindAndTestComponents()
    {
        ...
        _playerWeaponsScripts = Tools.GetComponentWithAssertion<FPCPlayerWeapons>(_player);
        _playerPointsScript = Tools.GetComponentWithAssertion<PlayerPoints>(_player);
        _playerFootstepSoundsScripts = Tools.GetComponentWithAssertion<FootstepSounds>(_player);
        _playerTakeDamageScripts = Tools.GetComponentWithAssertion<PlayerTakeDamage>(_player);
        _characterStats = Tools.GetComponentInChildrenWithAssertion<CharacterStats>(_player);
        ...
        Tools.CheckNull<StarterAssets.StarterAssetsInputs>(_inputManager, nameof(_inputManager), gameObject);
    ...
    private void Initialize()
    {
        ...
        _inputManager.Initialize(_IUIManagerInput);
    ...
    private void SetAnalyticsStepFirstWorld2() => _analytics.TakeNextFirstStep(4);
    ...
    {
        _analytics.TakeNextFirstStep(9);
    ...
    {
        _analytics.TakeNextFirstStep(8);
    }
    ...
    public bool ToShowTutorialInput1()
    {
        if (_gameSettingsSO.IsTutorialWorld2Input1)
        {
            return false;
        }
        else
        {
            _gameSettingsSO.IsTutorialWorld2Input1 = true;
            return true;
        }
    }
